public class SAES {
    ...
    public static String encryptASCII(String plaintext, String keyStr) {
        // 加密ASCII码
        plaintext = plaintext.replaceAll("\\s", ""); // 移除空格
        StringBuilder binaryPlaintext = new StringBuilder();
        for (char ch : plaintext.toCharArray()) {
            binaryPlaintext.append(charToBinary(ch));
        }
        String ciphertextBinary = encrypt(binaryPlaintext.toString(), keyStr);
        // 将加密后的二进制字符串转换为ASCII字符
        StringBuilder ciphertextASCII = new StringBuilder();
        for (int i = 0; i < ciphertextBinary.length(); i += 8) {
            String byteBinary = ciphertextBinary.substring(i, i + 8);
            ciphertextASCII.append(binaryToChar(byteBinary));
        }
        return ciphertextASCII.toString();
    }

    public static String decryptASCII(String ciphertext, String keyStr) {
        // 解密ASCII码
        StringBuilder binaryCiphertext = new StringBuilder();
        for (char ch : ciphertext.toCharArray()) {
            binaryCiphertext.append(charToBinary(ch));
        }
        String plaintextBinary = decrypt(binaryCiphertext.toString(), keyStr);
        // 将解密后的二进制字符串转换为ASCII字符
        StringBuilder plaintextASCII = new StringBuilder();
        for (int i = 0; i < plaintextBinary.length(); i += 8) {
            String byteBinary = plaintextBinary.substring(i, i + 8);
            plaintextASCII.append(binaryToChar(byteBinary));
        }
