...
public class ExtendedSAES extends SAES {
    ...
    public static int generateRandomIV() {
        ...
        return random.nextInt(0x10000); // 生成0到0xFFFF之间的随机数
    }
    ...
    public static String cbcDecrypt(List<Integer> ciphertextBlocks, String keyStr, int iv) {
        ...
        for (Integer ciphertextBlock : ciphertextBlocks) {
            // 将整数密文块转换为二进制字符串
            String binaryCiphertextBlock = ToBinary(ciphertextBlock, 16);
            // 解密当前密文块
            String decryptedBlock = decrypt(binaryCiphertextBlock, keyStr);
            // 将解密块转换为整数
            ...
            // 将得到的明文块转换为十六进制并添加到明文字符串
            plaintextBuilder.append(String.format("%04X", plaintextBlock));
            // 更新前一个块为当前密文块
            previousBlock = ciphertextBlock;
        }

        // 提取每组的最后一个十六进制字符并组合
        String hexResult = plaintextBuilder.toString();
        StringBuilder combinedHexBuilder = new StringBuilder();
        for (int i = 3; i < hexResult.length(); i += 4) {
            combinedHexBuilder.append(hexResult.charAt(i));
        }

        // 将组合后的十六进制字符串转换为二进制
        String combinedHex = combinedHexBuilder.toString();
        String binaryResult = Integer.toBinaryString(Integer.parseInt(combinedHex, 16));

        // 去掉前面的0并只保留后16位
        binaryResult = binaryResult.length() >= 16 ? binaryResult.substring(binaryResult.length() - 16) : binaryResult;
